import excel "C:\Users\15253\Desktop\毕业论文\数据\did1.xlsx", sheet("Sheet1") firstrow clear
save data10.dta, replace

//数据处理
gen did = 1 if Year>2015 & industy == 0
replace did = 0 if did == .
gen taxa = 0.25 if gaoxin==0
replace taxa =0.15 if gaoxin== .
gen taxb = 0.5 if Year > 2015 & industy == 0
replace taxb = 0.5 if industy== 0
replace taxb = 0 if taxb == .
gen jiaohu = did *taxa*taxb
gen jiaohu1 = did
gen jiaohu2 = did
gen size=ln(size1)
...
gen invia1=Invia1+Umia1
gen Invia=ln(Invia1+1)
gen invia=ln(invia1+1)
encode Scode,gen(id)
...
use data10.dta,clear
reghdfe Invia jiaohu ,absorb(Scode Year)
est sto m1
...
outreg2 [m1 m2 m3 m4 m5] using 基准回归10.xls, replace  dec(4) addtext(Company FE, YES,Year FE, YES)
...
graph export 平行趋势检验10-1.tif, replace
...
outreg2 [m1 m2 m3 m4] using 稳健性检验10-1.xls, replace  dec(4) addtext(Company FE, YES,Year FE, YES)
...
est sto m2
reghdfe Invia jiaohu1 lev Top income size number roa groincome TBq wct tax
est sto m3
reghdfe Invia jiaohu1 lev Top income size number roa groincome TBq wct tax,absorb(Scode Year)
est sto m4
reghdfe Invia jiaohu2 lev Top income size number roa groincome TBq wct tax
est sto m5
...
outreg2 [m1 m2 m3 m4 m5 m6] using 稳健性检验10-2.xls, replace  dec(3) addtext(Company FE, YES,Year FE, YES)


//稳健性检验3 加入城市固定效应

reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax,absorb(Scode Year city)
est sto m1
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax,absorb(Scode Year Year#city1)
est sto m2
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax,absorb(Scode Year indcode)
est sto m3
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax,absorb(Scode Year Year#indcode)
est sto m4

esttab m1 m2 m3 m4, b(%6.4f) se(%6.4f)  star(* 0.1 ** 0.05 *** 0.01)  scalar(N r2) compress nogaps
outreg2 [m1 m2 m3 m4] using 稳健性检验10-3.xls, replace  dec(4) addtext(Company FE, YES,Year FE, YES)
...
est sto m1
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if Year<2019 ,absorb(Scode) 
est sto m2
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if Year<2019 ,absorb(Year)
est sto m3
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if Year<2019 ,absorb(Scode Year)
est sto m4
esttab m1 m2 m3 m4, b(%6.4f) se(%6.4f)  star(* 0.1 ** 0.05 *** 0.01)  scalar(N r2) compress nogaps
outreg2 [m1 m2 m3 m4] using 稳健性检验10-4.xls, replace  dec(4) addtext(Company FE, YES,Year FE, YES)
...
forvalues i=1/500{
 ...
 qui merge 1:m id using  data10.dta 
 gen treat = (_merge == 3)  
 gen period = (Year >= 2016)  
 gen didd = treat*period*taxa*taxb
...
xlabel(-0.5(4)0.5) ylabel(,angle(0)) ///
xline( 2.7951, lwidth(vthin) lp(shortdash)) xtitle("系数") ///
...
esttab m1 m2 m3 m4 m5, b(%6.4f) se(%6.4f)  star(* 0.1 ** 0.05 *** 0.01)  scalar(N r2) compress nogaps
outreg2 [m1 m2 m3 m4 m5] using 稳健性检验10-4时间.xls, replace  dec(4) addtext(Company FE, YES,Year FE, YES)
...
est sto m1
reghdfe Invia jiaohu $conval if common==1 , absorb(id) 
est sto m2
reghdfe Invia jiaohu $conval if common==1 , absorb(Year) 
est sto m3
reghdfe Invia jiaohu $conval if common==1 , absorb(id Year) 
est sto m4
esttab m1 m2 m3 m4, b(%6.4f) se(%6.4f)  star(* 0.1 ** 0.05 *** 0.01)  scalar(N r2) compress nogaps
outreg2 [m1 m2 m3 m4] using 稳健性检验7-5PSM-DID.xls, replace  dec(4) addtext(Company FE, YES,Year FE, YES)
...
sort id Year
gen Invia11 = .
by id: replace Invia11 = Invia[_n+1] if Year != Year[_n+1] & id == id[_n+1]

reghdfe Invia1 jiaohu lev Top income size number roa groincome TBq wct tax
est sto m1
reghdfe Invia1 jiaohu lev Top income size number roa groincome TBq wct tax ,absorb(Scode) 
est sto m2
reghdfe Invia1 jiaohu lev Top income size number roa groincome TBq wct tax ,absorb(Year)
est sto m3
reghdfe Invia1 jiaohu lev Top income size number roa groincome TBq wct tax ,absorb(Scode Year)
est sto m4
esttab m1 m2 m3 m4, b(%6.4f) se(%6.4f)  star(* 0.1 ** 0.05 *** 0.01)  scalar(N r2) compress nogaps
outreg2 [m1 m2 m3 m4] using 稳健性检验10-8.xls, replace  dec(4) addtext(Company FE, YES,Year FE, YES)
...
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if region==1 
est sto m1
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if region==1 , absorb(Scode Year) 
est sto m2
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if region==2 
est sto m3
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if region==2 , absorb(Scode Year) 
est sto m4
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if region==3 
est sto m5
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if region==3 , absorb(Scode Year) 
est sto m6
esttab m1 m2 m3 m4 m5 m6, b(%6.4f) se(%6.4f)  star(* 0.1 ** 0.05 *** 0.01)  scalar(N r2) compress nogaps
outreg2 [m1 m2 m3 m4 m5 m6] using 异质性分析1.xls, replace  dec(4) addtext(Company FE, YES,Year FE, YES)

cd C:\Users\15253\Desktop\毕业论文\数据
use data7.dta,clear
//异质性分析2国企
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if soe==1 
est sto m1
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if soe==1 , absorb(Scode Year) 
est sto m2
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if soe==0
est sto m3
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if soe==0 , absorb(Scode Year) 
est sto m4
esttab m1 m2 m3 m4, b(%6.4f) se(%6.4f)  star(* 0.1 ** 0.05 *** 0.01)  scalar(N r2) compress nogaps
outreg2 [m1 m2 m3 m4] using 异质性分析2.xls, replace  dec(4) addtext(Company FE, YES,Year FE, YES)

//异质性分析3高新

reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if gaoxin==0
est sto m1
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if gaoxin==0 , absorb(Scode Year) 
est sto m2
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if gaoxin==1
est sto m3
reghdfe Invia jiaohu lev Top income size number roa groincome TBq wct tax if gaoxin==1 , absorb(Scode Year) 
est sto m4
esttab m1 m2 m3 m4, b(%6.4f) se(%6.4f)  star(* 0.1 ** 0.05 *** 0.01)  scalar(N r2) compress nogaps
outreg2 [m1 m2 m3 m4] using 异质性分析3.xls, replace  dec(4) addtext(Company FE, YES,Year FE, YES)
...
esttab m1 m2 m3 m4, b(%6.4f) se(%6.4f)  star(* 0.1 ** 0.05 *** 0.01)  scalar(N r2) compress nogaps
outreg2 [m1 m2 m3 m4] using 异质性分析4.xls, replace  dec(4) addtext(Company FE, YES,Year FE, YES)
...
esttab m1 m2 m3 m4, b(%6.4f) se(%6.4f)  star(* 0.1 ** 0.05 *** 0.01)  scalar(N r2) compress nogaps
outreg2 [m1 m2 m3 m4] using 机制分析前1.xls, replace  dec(4) addtext(Company FE, YES,Year FE, YES)
...
esttab m1 m2 m3 m4 m5, b(%6.4f) se(%6.4f)  star(* 0.1 ** 0.05 *** 0.01)  scalar(N r2) compress nogaps
outreg2 [m1 m2 m3 m4 m5] using 机制分析后.xls, replace  dec(4) addtext(Company FE, YES,Year FE, YES)
